...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($recipe_id_err) && empty($recipe_name_err) && empty($ingredients_err) && empty($preparation_instructions_err)  && empty($servings_err) && empty($location_err) && empty($category_err)){
        ...
        $sql = "INSERT INTO recipes_table (recipe_id, recipe_name, ingredients, preparation_instructions, servings, location, category) VALUES (?, ?, ?, ?, ?, ?, ?)";

        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssssss", $param_recipe_id, $param_recipe_name, $param_ingredients, $param_preparation_instructions, $param_servings, $param_location, $param_category);
